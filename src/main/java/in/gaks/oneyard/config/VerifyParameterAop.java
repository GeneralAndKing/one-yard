...
import in.gaks.oneyard.model.helper.VerifyParameterPredicate;
...
public class VerifyParameterAop {
   ...
   * 切点.
   */
  @Pointcut("@annotation(in.gaks.oneyard.model.helper.VerifyParameter)")
  ...
  public void verify(JoinPoint joinPoint) throws NoSuchMethodException {
    ...
    check(annotation.number(), (condition, value) -> Objects.nonNull(value)
        ...
        () -> " 不存在或不为数字类型！");
    check(annotation.is(), (condition, value) -> Objects.nonNull(value)
  ...
  private String getField(String expression) {
    return (StringUtils.substringBefore(StringUtils.substringBefore(
        expression,
        FIELD_MARK),
        TIP_MARK)).trim();
  }
  ...
  private String getCondition(String expression) {
    return (expression.contains(FIELD_MARK) || expression.contains(TIP_MARK))
        ?
        (StringUtils.substringAfter(StringUtils.substringBefore(
            expression,
            TIP_MARK),
            FIELD_MARK)).trim()
        : null;
  ...
  private Object getValue(String field) {
    ...
    JSONObject p = this.param;
    for (String key : keys) {
      p = p.getJSONObject(key);
      Assert.notNull(p, String.format("%s 为必填项！", key));
    }
    return p.get(target);
  }
